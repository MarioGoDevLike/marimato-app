...
class NumberTable extends React.Component {
  ...
  handleCellClick = (rowIndex, colIndex) => {
    ...
    if (selectedRow) {
      if (clickedNumber !== null && rowIndex === this.state.selectedRow) {
        ...
        updatedNumbers[rowIndex] = updatedNumbers[rowIndex].map(
          (number, index) => (index === colIndex ? null : number)
        );
    ...
    if (availableNumbers.length > 0) {
      ...
      if (
        ...
        const updatedNumbers = this.state.numbers.map((row, rIndex) =>
          row.map((num, cIndex) =>
            cIndex === selectedColumn &&
            rIndex === rowIndex &&
            num === pcChosenNumber.number
              ? null
              : num
          )
        );
      ...
      const updatedNumbers = this.state.numbers.map((row, rIndex) =>
        row.map((num, cIndex) =>
          cIndex === highestNumber.colIndex &&
          rIndex === highestNumber.rowIndex &&
          num === highestNumber.number
            ? null
            : num
        )
      );
      ...
      alert(
        `Bot chose the highest available number ${
          highestNumber.number
        }, column:${highestNumber.colIndex + 1}, row: ${
          highestNumber.rowIndex + 1
        }`
      );
      this.setState({ isPlayerTurn: true });
    }
  ...
  render() {
    const { selectedColumn, hoveredRow, isPlayerTurn, selectedRow } =
      this.state;
        ...
        <div className="First-part">
          ...
          <div className="Totals">
            <div className="Total-PC">
              Player 1 Total Value: {this.state.playerTotalValue}
            </div>
            <h3 className="Total-PC Turn-style" style={{ color: "#2F697B" }}>
              {isPlayerTurn ? "Your turn" : "Computer's turn"}
            </h3>
            <div className="Total-PC PC-Value">
              PC Total Value: {this.state.pcTotalValue}
            </div>
          </div>
        </div>
              ...
              >
                {rowNumbers.map((number, colIndex) => (
                  <td
                    ...
                    key={colIndex}
                    style={
                      colIndex === selectedColumn && !isPlayerTurn
                        ? { backgroundColor: "grey" }
                        : rowIndex === (selectedRow || hoveredRow) &&
                          isPlayerTurn
                        ? { backgroundColor: "grey" }
                        : {}
                    }
